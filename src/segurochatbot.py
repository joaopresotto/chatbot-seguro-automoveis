...
class ChatbotSeguros:
    def __init__(self, database_path=os.path.join('data','chroma_database'), 
        ...
        self.model_oraculo = 'mistral:latest' # parece funcionar melhor para gerar o json de avaliação!
    ...
    def avaliar_resposta(self, query, resposta, contexto):
        ...
        try:
            ...
            1. **Relevância**: A resposta está alinhada com o tema da pergunta e o contexto? O chatbot evitou divergir para tópicos não relacionados?
            2. **Precisão**: A resposta contém informações corretas e específicas? Há ausência de erros ou informações enganosas?
            3. **Clareza**: A resposta está bem estruturada e é compreensível? Há ambiguidades ou termos inadequados?
            4. **Coerência e Consistência**: O chatbot manteve o mesmo idioma e contexto durante a interação? Não houve mudanças inesperadas no idioma ou no escopo da resposta?
            5. **Exatidão no escopo**: A resposta do chatbot permaneceu dentro do domínio esperado (seguros de automóveis das seguradoras Santander, Bradesco, Porto Seguro e Suhai)?
            ...
            try:
                ...
                required_keys = ['relevante', 'preciso', 'claro', 'coerente', 'score', 'feedback']
